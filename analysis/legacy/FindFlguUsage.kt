    ...
    for (entry in entries) {
        ...
        for (clazz in dexFile.classes) {
            for (method in clazz.methods) {
                ...
                val hasFlgu = impl.instructions.any { ins ->
                    (ins as? ReferenceInstruction)?.reference?.toString()?.contains("flgu") == true
                }
                if (hasFlgu) {
                    println("Class ${clazz.type}->${method.name} instantiates or references flgu")
                }
            }
        }
    }
